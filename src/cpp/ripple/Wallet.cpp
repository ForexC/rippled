...
void Wallet::startStandalone()
{
	nodeIdentityCreateDummy();
	if (!nodeIdentityLoad())
		throw std::runtime_error("unable to retrieve new node identity.");
}
...
// Create and store a network identity.
bool Wallet::nodeIdentityCreateDummy() {
	if (!theConfig.QUIET)
		std::cerr << "NodeIdentity: Creating dummy." << std::endl;
	...
	ScopedLock sl(theApp->getWalletDB()->getDBLock());
	db->executeSQL(str(boost::format("INSERT INTO NodeIdentity (PublicKey,PrivateKey,Dh512,Dh1024) VALUES ('%s','%s',%s,%s);")
		% "n94BBBssez9JvsjBk4Jc7SjZCAGovgRw9zbTAB2Yh4Yu1v8L4epf"
		% "paWo5keUmqyqyd7rRAEzmZL9aP885vxSnpYY8xLFcM3s6ZqU2WF"
		% sqlEscape(strDh512)
		% sqlEscape(strDh1024)));
	// XXX Check error result.

	if (!theConfig.QUIET)
		std::cerr << "NodeIdentity: Created dummy." << std::endl;
